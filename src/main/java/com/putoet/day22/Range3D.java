...
record Range3D(@NotNull Point3D min, @NotNull Point3D max) {
    ...
    public static List<Range3D> of(@NotNull List<String> lines) {
        return lines.stream().map(Range3D::of).toList();
    }

    public static Range3D of(@NotNull String line) {
    ...
    public static Range3D of(@NotNull Point3D from, @NotNull Point3D to) {
        return new Range3D(from, to);
    }

    public static long point3DCount(@NotNull Range3D range) {
        return (long) Math.abs(1 + range.max.x() - range.min.x()) *
                Math.abs(1 + range.max.y() - range.min.y()) *
                Math.abs(1 + range.max.z() - range.min.z());
    }

    public static List<Range3D> carve(@NotNull Range3D toSplit, @NotNull Range3D other) {
        final var split = new ArrayList<Range3D>();
        ...
        if (toSplit.min().x() <= other.min().x() && other.min().x() <= toSplit.max().x()) {
            ...
            final var temp = toSplit.maxX(other.min().x() - 1);
            if (temp.isPresent()) {
                split.add(temp.get());
                toSplit = toSplit.minX(other.min().x()).orElseThrow();
            }
        }
        ...
        if (toSplit.min().x() <= other.max().x() && other.max().x() <= toSplit.max().x()) {
            // Add block that ends at the minX of the next command range
            final var temp = toSplit.minX(other.max().x() + 1);
            if (temp.isPresent()) {
                split.add(temp.get());
                toSplit = toSplit.maxX(other.max().x()).orElseThrow();
            }
        }
        ...
        if (toSplit.min().y() <= other.min().y() && other.min().y() <= toSplit.max().y()) {
            // Add block that ends at the minX of the next command range
            final var temp = toSplit.maxY(other.min().y() - 1);
            if (temp.isPresent()) {
                split.add(temp.get());
                toSplit = toSplit.minY(other.min().y()).orElseThrow();
            }
        }
        ...
        if (toSplit.min().y() <= other.max().y() && other.max().y() <= toSplit.max().y()) {
            // Add block that ends at the minX of the next command range
            final var temp = toSplit.minY(other.max().y() + 1);
            if (temp.isPresent()) {
                split.add(temp.get());
                toSplit = toSplit.maxY(other.max().y()).orElseThrow();
            }
        }
        ...
        if (toSplit.min().z() <= other.min().z() && other.min().z() <= toSplit.max().z()) {
            // Add block that ends at the minX of the next command range
            final var temp = toSplit.maxZ(other.min().z() - 1);
            if (temp.isPresent()) {
                split.add(temp.get());
                toSplit = toSplit.minZ(other.min().z()).orElseThrow();
            }
        }
        ...
        if (toSplit.min().z() <= other.max().z() && other.max().z() <= toSplit.max().z()) {
            // Add block that ends at the minX of the next command range
            final var temp = toSplit.minZ(other.max().z() + 1);
            if (temp.isPresent()) {
                split.add(temp.get());
                toSplit.maxZ(other.max().z()).orElseThrow();
            }
        }
        return split;
    }
    ...
    public List<Range3D> carve(@NotNull Range3D other) {
