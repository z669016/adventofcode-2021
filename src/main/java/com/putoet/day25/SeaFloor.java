...
public record SeaFloor(char[][] grid) {
    public static final char EAST = '>';
    public static final char SOUTH = 'v';
    public static final char EMPTY = '.';

    public static SeaFloor of(List<String> lines) {
        assert lines != null;
    ...
    public Pair<SeaFloor, Integer> step() {
        ...
        final SeaFloor copyTwo = copyOne.copy();
        for (int y = 0; y < copyOne.grid.length; y++)
            for (int x = 0; x < copyOne.grid[y].length; x++)
    ...
    private int move(int x, int y, char toMove) {
        ...
        if (grid[y][x] == EAST) {
            int nx = (x == grid[y].length - 1 ? 0 : x + 1);
            if (isEmpty(nx, y)) {
                grid[y][x] = EMPTY;
                grid[y][nx] = EAST;
                return 1;
